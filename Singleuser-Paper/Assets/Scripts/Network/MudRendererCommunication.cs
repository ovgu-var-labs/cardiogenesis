...
public class MudRendererCommunication : MonoBehaviour
{
    ...
    public List<GameObject> studentMudRenderers;            // The mudrenderes of the students
    ...
    void Update()
    {
        ...
        if (!markingIsLocked && studentMudRenderers.Count > 1)
        {
            // GrabValuesMarkierung();
            // GrabSetValuesNotations();
        }
    ...
    private void AssignMudrenderes()
    {
        for (int i = 0; i < MudRenderers.Length; i++)
        {
            ...
            {
                studentMudRenderers.Add(MudRenderers[i]);
            }
        }
    }
    ...
    */
    ...
    private void GrabValues()
    {
        ...
        animationProgress = moderatorButtons.AnimationSlider.SliderValue;
    ...
    */
    ...
    /// <summary>
    /// Called for students.
    /// Should rotate mud renderer of students in a way that they see own heart in same angle as moderator sees his heart
    /// </summary>
    /// <param name="angleToAchieve">Angle of moderator</param>
    /// <param name="heartToRotate">Mudrender of students</param>
    public void ForceRotateHeart(float angleToAchieve, GameObject heartToRotate)
    {
    ...
    */
    ...
    public void SpreadMarkierungen(List<GameObject> markierungsListe, GameObject parent)
    {
        foreach(GameObject markierung in markierungsListe)
        {
            GameObject tmp = Instantiate(otherHighlightPrefab, markierung.transform.localPosition, transform.localRotation, parent.transform);
            Destroy(tmp, 5f);
        }
    }
    ...
    */
    ...
    private void MoveNotationActive(int numberRenderer, GameObject gameObjectToMove)
    {
        ...
        // gameObjectToMove.transform.localScale = pukOriginal.transform.localScale;
        // gameObject.transform.localScale = new Vector3(correctScale, correctScale, correctScale);
    }

    public bool GetRotationsIsLocked()
    {
        // ModeratorMudRender.GetComponent<PlayerManager>().GetIsModerator()
