...
public class AnchorTutorialUIManager : MonoBehaviour
{
    ...
    // Created by Jonas
    public void CreateAnchorAt(Vector3 pos, Quaternion rot)
    {
        GameObject gs = PlaceAnchor(_saveableAnchorPrefab, pos, rot); //anchor A
        _workingAnchor = gs.AddComponent<OVRSpatialAnchor>();

        CreateAnchor(_workingAnchor, true);
        Debug.Log("Created Anchor at " + pos);
    }

    public int Get_allSavedAnchorsLength()
    {
        return _allSavedAnchors.Count;
    }

    public OVRSpatialAnchor GetActiveAnchor()
    {
        return _allRunningAnchors[0];
    }
